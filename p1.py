import os
import pickle
import re
import warnings
from flask import Flask, render_template, request
from sklearn.feature_extraction.text import TfidfVectorizer
from PIL import Image
import pytesseract
import PyPDF2
import pandas as pd
from sklearn.exceptions import InconsistentVersionWarning

warnings.filterwarnings("ignore", category=InconsistentVersionWarning)

app = Flask(__name__)

# Load the Random Forest classifier and TfidfVectorizer
with open("rf_classifier_categorization.pkl", "rb") as classifier_file:
    rf_classifier = pickle.load(classifier_file)

with open("tfidf_vectorizer_categorization.pkl", "rb") as vectorizer_file:
    tfidf_vectorizer = pickle.load(vectorizer_file)

# Load the job recommendation dataset
job_recommendation_data = pd.read_csv("jobs_dataset_with_features.csv")
job_recommendation_data['Features'] = job_recommendation_data['Features'].fillna("").str.lower()

# Helper function to clean the resume text
def clean_resume(text):
    text = re.sub(r'\n+', ' ', text)  # Replace newline characters with spaces
    text = re.sub(r'\s+', ' ', text)  # Replace multiple spaces with a single space
    text = re.sub(r'[^\w\s]', '', text)  # Remove punctuation
    return text.strip()

# Function to extract text from files
def extract_text(file):
    try:
        if file.filename.endswith(".txt"):
            return file.read().decode("utf-8")
        elif file.filename.endswith(".pdf"):
            pdf_reader = PyPDF2.PdfReader(file)
            text = ""
            for page in pdf_reader.pages:
                text += page.extract_text() or ""
            return text
        elif file.filename.endswith((".jpeg", ".jpg", ".png")):
            image = Image.open(file)
            return pytesseract.image_to_string(image)
        else:
            return ""
    except Exception as e:
        print(f"Error extracting text: {e}")
        return ""

# Extract name from the resume text
def extract_name(text):
    name_match = re.findall(r'\b([A-Z][a-z]+(?: [A-Z][a-z]+)+)\b', text)
    if name_match:
        return name_match[0]  # Return the first match (assumed to be the name)
    return "Name not found"

# Extract work experience from the resume text
def extract_experience(text):
    experience_keywords = ["experience", "work history", "employment", "professional experience"]
    lines = text.split("\n")
    experience_found = False
    experience_lines = []
    
    for line in lines:
        if any(keyword in line.lower() for keyword in experience_keywords):
            experience_found = True
            continue  # Skip this line as it is just the section title
        if experience_found:
            if line.strip() == "":
                break
            experience_lines.append(line.strip())
    
    return " ".join(experience_lines) if experience_lines else "Work experience not found"

# Prediction function
def predict_category_with_rf(resume_text):
    vectorized_text = tfidf_vectorizer.transform([resume_text])
    prediction = rf_classifier.predict(vectorized_text)[0]
    return prediction

# Recommend jobs function
def recommend_jobs(category, skills=None):
    # Filter based on the category
    recommended_jobs = job_recommendation_data[job_recommendation_data['Role'].str.lower() == category.lower()]
    
    # Filter further based on skills if provided
    if skills:
        skills = skills.lower()
        recommended_jobs = recommended_jobs[recommended_jobs['Features'].str.contains(skills, na=False)]
    
    # Return top 5 recommendations
    return recommended_jobs.head(5)

# Home route
@app.route('/')
def home():
    return render_template('index.html')

# Resume upload and prediction route
@app.route('/predict', methods=['POST'])
def predict():
    if 'resume' not in request.files:
        return "No file part"
    file = request.files['resume']
    if file.filename == '':
        return 'No selected file'

    resume_text = extract_text(file)
    if not resume_text.strip():
        return "Unable to extract text from the file. Please upload a valid file."

    resume_text = clean_resume(resume_text)
    predicted_category = predict_category_with_rf(resume_text)

    # Extract name and experience
    extracted_name = extract_name(resume_text)
    extracted_experience = extract_experience(resume_text)

    # Recommend jobs from the new dataset
    recommended_jobs = recommend_jobs(predicted_category, extracted_experience)

    return render_template(
        'result.html',
        prediction=predicted_category,
        name=extracted_name,
        experience=extracted_experience,
        jobs=recommended_jobs.to_dict(orient='records')  # Pass recommended jobs
    )

if __name__ == "__main__":
    app.run(debug=True)
